...
class AdicionarNovaRotinaReserva(unittest.TestCase):
    ...
    def test_apagar_rotina(self):
        ...
        rotinas_nome_antes_adicionar = driver.find_elements(by=AppiumBy.ANDROID_UIAUTOMATOR,
                                                            value="new UiSelector().description(\"Teste Hipertrofia\")")

        if len(rotinas_nome_antes_adicionar) == 0:
            adicionar_button = (AppiumBy.XPATH, "//android.widget.Button[@text='Adicionar']")
            WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(adicionar_button))
            driver.find_element(*adicionar_button).click()
            nova_rotina_button = (AppiumBy.ACCESSIBILITY_ID, "Nova Rotina")
            ...
            time.sleep(2)
            driver.swipe(470, 1400, 470, 900, 400)
            salvar_button = driver.find_element(by=AppiumBy.ANDROID_UIAUTOMATOR,
                                                value="new UiSelector().text(\"Salvar\")")
            salvar_button.click()
            rotina_criada = (AppiumBy.XPATH, "(//android.view.View/following-sibling::android.widget.TextView)[1]")
            WebDriverWait(driver, wait_time).until_not(EC.presence_of_element_located(loading))
            WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(rotina_criada))
            ...
            voltar_button = (AppiumBy.ANDROID_UIAUTOMATOR, "new UiSelector().text(\"  "
                                                           "Voltar\")")
            WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(voltar_button))
            element = driver.find_element(*voltar_button)
            element.click()
            WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(adicionar_button))

        rotinas_nome_antes_apagar = driver.find_elements(by=AppiumBy.ANDROID_UIAUTOMATOR,
                                                         value="new UiSelector().description(\"Teste Hipertrofia\")")
        print(f">>>>>>>>>>>>>>>>>>>> rotinas_nome_antes: {len(rotinas_nome_antes_apagar)}")
        opcoes_button = driver.find_element(by=AppiumBy.ANDROID_UIAUTOMATOR,
                                            value="new UiSelector().text(\"\").instance(0)")
        opcoes_button.click()
        excluir_button = (AppiumBy.ACCESSIBILITY_ID, "Excluir")
        WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(excluir_button))

        element = driver.find_element(*excluir_button)
        element.click()
        pop_up_confirmacao = (AppiumBy.ANDROID_UIAUTOMATOR, "new UiSelector().text(\"Sim, pode excluir!\")")
        WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(pop_up_confirmacao))
        element = driver.find_element(*pop_up_confirmacao)
        element.click()
        # aguardar_rotinas_nome_depois = (by=AppiumBy.ANDROID_UIAUTOMATOR, "new UiSelector().description(\"Teste Hipertrofia\")")
        WebDriverWait(driver, wait_time).until(EC.presence_of_element_located(adicionar_button))
        rotinas_nome_depois = driver.find_elements(by=AppiumBy.ANDROID_UIAUTOMATOR,
                                                   value="new UiSelector().description(\"Teste Hipertrofia\")")
        self.assertEqual(len(rotinas_nome_depois), len(rotinas_nome_antes_apagar) - 1,
                         f'O valor esperado era "{rotinas_nome_depois}", mas o encontrado foi "{rotinas_nome_antes_apagar}"')
